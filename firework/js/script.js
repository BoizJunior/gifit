...
// --- BỔ SUNG: Lời chúc xuất hiện ---
const phrases = [
  'NĂM MỚI',
  'THẬT Ý NGHĨA',
  'NHIỀU MAY MẮN',
  'HẠNH PHÚC NHÉ',
  'EM XINH MÀ', // Đã đổi từ Em xinh lắm
  'ĐỪNG KHÓC NHÉ',
  'PHẢI TÍCH CỰC',
  'VUI VẺ NHÉ ❤️', // Đã đổi từ Yêu em nhiều
]
let phraseIndex = 0
...
function handleStateChange(state, prevState) {
  ...
  const bgMusic = document.getElementById('bg-music') // Lấy file nhạc nền
...
// --- BỔ SUNG: Hàm chuyển văn bản thành các điểm tọa độ ---
function getTextPixels(text, fontSize) {
  const canvas = document.createElement('canvas')
  const ctx = canvas.getContext('2d')

  // Dùng font Russo One đã có sẵn trong project
  ctx.font = `900 ${fontSize}px "Russo One", arial`
...
function seqText() {
  ...
  BurstFlash.add(centerX, centerY, fontSize * 2)
...
function startSequence() {
  ...
  // 1. HIỆU ỨNG CHỮ: Chiếm 25% tỷ lệ (từ 0.00 đến 0.25)
  if (rand < 0.25) {
    return seqText()
  }

  // 2. MƯA PHÁO HOA: Chiếm khoảng 8% (từ 0.25 đến 0.33)
  if (
    rand < 0.33 &&
  ...
  // 3. KIM TỰ THÁP: Chiếm 2% (từ 0.33 đến 0.35)
  if (rand < 0.35) {
    return seqPyramid()
  }

  // 4. PHÁO ĐƠN NGẪU NHIÊN: Chiếm 35% (từ 0.35 đến 0.70)
  if (rand < 0.7 && !IS_HEADER) {
    return seqRandomShell()
  }

  // 5. PHÁO ĐÔI: Chiếm 15% (từ 0.70 đến 0.85)
  else if (rand < 0.85) {
    return seqTwoRandom()
  }

  // 6. PHÁO BA: Chiếm 15% còn lại (từ 0.85 đến 1.0)
  else {
    return seqTriple()
  }
}
